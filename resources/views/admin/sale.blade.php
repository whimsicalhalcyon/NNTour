    ...
    <div class="container p-0">
        <div class="container-registration">
            <div class="container-fluid">
                <h1 class="main-text mb-5">Акции</h1>
                <button class="btn btn-secondary reg-btn" data-bs-toggle="modal" data-bs-target="#addSale">Добавить акцию</button>
            </div>

            {{--            Модалка с добавлением самого тура    --}}
            <div class="modal fade modal-main" id="addSale" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content modal-main" style="border:none; border-radius: 20px">
                            ...
                            <div class="modal-body">
                                <div class="form-floating mb-3">
                                    ...
                                    <label for="floatingInput" class="form-input">Название акции</label>
                                    @error('title')
                                ...
                                <div class="form-floating mb-3">
                                    ...
                                    <label for="floatingInput" class="form-input">Описание акции</label>
                                    @error('title')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
            ...
            {{--            Список туров  --}}
            <div class="tours-list mt-3 row row-cols-1 row-cols-md-3 g-4">
                @foreach($sales as $sale)
                    <div class="col">
                        <div class="card tour-card" style="width: 100%">
                            <div class="card-image-container">
                                <img src="{{ $sale->img }}" class="card-img-top">

                            </div>
                            <div class="card-body">
                                <h5 class="card-title">{{ $sale->name }}</h5>
                                <p class="card-text text-muted description-truncate">{{ $sale->description }}</p>

                                <div class="btn-panel d-flex gap-2">
                                    <button class="btn btn-outline-primary btn-edit main-button text-white flex-grow-1"  data-bs-toggle="modal" data-bs-target="#editSale{{$sale->id}}">Редактировать</button>
                                    <form action="{{route('admin.sale.destroy', ['sale'=>$sale])}}" method="post">
                                        @csrf
                                        @method('delete')
                    ...
                    <div class="modal fade modal-main" id="editSale{{$sale->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <form action="{{route('admin.sale.update', ['sale'=>$sale])}}" method="post">
                                @csrf
                                @method('put')
                                <div class="modal-content modal-main" style="border:none; border-radius: 20px">
                                    <div class="modal-header border-0">
                                        <h1 class="modal-title fs-5" id="exampleModalLabel">Редактирование скидки</h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Закрыть"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control input-reg @error('title') is-invalid @enderror" id="floatingInput"
                                                   placeholder="name@example.com" name="title" value="{{$sale->title}}">
                                        ...
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control input-reg @error('description') is-invalid @enderror" id="floatingInput"
                                                   placeholder="name@example.com" name="description" value="{{$sale->description}}">
